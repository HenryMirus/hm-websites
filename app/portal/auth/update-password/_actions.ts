...
export async function updatePasswordAction(
  ...
  // 2. Recovery-Cookie verifizieren — stellt sicher dass der Zugriff
  //    ausschließlich über den signierten E-Mail-Link erfolgte
  ...
  if (!rawToken || !verifyRecoveryToken(rawToken, user.id)) {
    return {
      error:
        "Ungültiger oder abgelaufener Reset-Link. Bitte ein neues Passwort-Reset anfordern.",
    };
  }
  ...
  // 5. Alle Sessions global abmelden (alle Geräte, alle Tabs)
  //    Nutzt die Admin-API damit auch fremde Sessions widerrufen werden
